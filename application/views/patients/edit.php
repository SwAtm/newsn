...
echo form_open('patients/edit','',array('id'=> $id));
	//echo form_hidden($data);
	foreach ($patients as $patients1):
	$cell=array('data'=>(isset($patients1['options'])? (form_dropdown($patients1['name'],$patients1['options'],isset($patients1['value'])? $patients1['value']: $patients1['name'])):
	(form_input(array('name'=>$patients1['name'], 'maxlength'=>$patients1['max_len'], 'size'=>$patients1['max_len'],'value'=> $patients1['value'])))), 'colspan'=>'4');
