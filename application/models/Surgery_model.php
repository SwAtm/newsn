...
class Surgery_model extends CI_Model{
	...
	//called by fitness/get_details_id, surgery/get_id_add, surgery/get_id_allot
	{
	...
	//called by surgery/get_date
	{
	$query=$this->db->select('opd.name, opd.add1, opd.add2, opd.taluq, opd.district, opd.phone, opd.dm, opd.htn, opd.dob, opd.sex, opd.language, surgery.id, surgery.gvp, surgery.eye, surgery.k1, surgery.k2, surgery.al, surgery.iol, surgery.rbs, surgery.ecg, surgery.sac, surgery.iop, surgery.remark, surgery.surgeon');
	$query=$this->db->from('surgery');
	$query=$this->db->join('opd','surgery.id=opd.id','inner');
	$query=$this->db->where('surgery.dos',$dos);
	$query=$this->db->order_by('opd.name','ASC');
	...
	//called by surgery/get_date
	{
	$query=$this->db->select('opd.name, opd.add1, opd.add2, opd.taluq, opd.district, opd.phone, opd.dm, opd.htn, opd.dob, opd.sex, surgery.id, surgery.ipno, surgery.gvp, surgery.eye, surgery.k1, surgery.k2, surgery.al, surgery.iol, surgery.surgeon ');
	$query=$this->db->from('surgery');
	$query=$this->db->join('opd','surgery.id=opd.id','inner');
	$query=$this->db->where('surgery.dos',$dos);
